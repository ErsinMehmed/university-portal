...
import { adStore, commonStore } from "@/stores/useStore";
...
const MainSection = () => {
  ...
  const { setErrorMessage, setErrorFields, errorFields, errorMessage } =
    commonStore;
  ...
  const handleFindJob = () => {
    const foundLocation = locations.find(
      (location) => location.label === adAdvacedSearchData.location
    );

    if (!adAdvacedSearchData.keyword || !adAdvacedSearchData.location) {
      setErrorMessage("Попълнете задължителните полета");
      setErrorFields("advancedSearch");
    } else if (!foundLocation) {
      setErrorMessage("Въведеният град не съществува");
      setErrorFields("advancedSearch");
    } else {
          ...
          <div className="w-full lg:w-7/12">
            ...
            >
              <h2 className="text-2xl sm:text-3xl md:text-4xl text-slate-700 font-bold font-mono text-center sm:text-left">
                Тук има <span className="text-[#1967d2]">93,178</span>{" "}
                публикации за теб!
              </h2>
              ...
              <div
                className={`lg:${
                  errorFields === "advancedSearch" ? "bg-red-200" : "bg-white"
                } lg:p-5 lg:shadow lg:border border-slate-200 rounded-lg lg:flex items-center lg:space-x-3 mt-8 space-y-5 lg:space-y-0`}
              >
                <div className="flex items-center w-full">
                  <div className="bg-white lg:bg-transparent py-3 sm:py-5 lg:py-0 rounded-l-lg pl-4 lg:pl-0 border border-gray-200 border-r-0 lg:border-0">
                    <HiOutlineMagnifyingGlass
                      className={`h-7 w-7 lg:mt-0.5 text-gray-400 ${
                        errorFields === "advancedSearch"
                          ? "text-red-500"
                          : "text-gray-400"
                      }`}
                    />
                  ...
                  <input
                    ...
                    onChange={(event) => handleInputChange("keyword", event)}
                    className={`text-gray-700 text-sm focus:outline-none w-full px-2.5 pl-3 sm:pl-4 lg:pl-2.5 py-4 sm:py-6 lg:py-2.5 ${
                      errorFields === "advancedSearch"
                        ? "bg-red-200 placeholder-red-500"
                        : "bg-white"
                    } border border-gray-200 border-l-0 lg:border-0 rounded-r-lg`}
                  />
                </div>

                <div className="flex items-center w-full lg:border-l-2 lg:pl-2.5">
                  <div
                    className={`bg-white lg:bg-transparent py-2.5 sm:py-[17px] lg:py-0 rounded-l-lg pl-4 lg:pl-0 border border-gray-200 border-r-0 lg:border-0`}
                  >
                    <CiLocationOn
                      className={`h-8 w-8 sm:mt-0.5 ${
                        errorFields === "advancedSearch"
                          ? "text-red-500"
                          : "text-gray-400"
                      }`}
                  ...
                  <input
                    type="text"
                    placeholder="град"
                    value={adAdvacedSearchData.location}
                ...
                <button
                  ...
                  className="w-full lg:w-auto flex justify-center text-white bg-blue-500 hover:bg-[#1967d2] hover:text-white font-semibold focus:outline-none rounded-lg py-3 lg:py-2.5 px-4 transition-all active:scale-95"
                >
                  Намери
                </button>
              </div>
